...
class HookInstallSmokeTest(unittest.TestCase):
    ...
    def test_ensure_todo_install_and_uninstall_use_family_directory(self) -> None:
        ...
        post_tool_use_script = (
            self.base_path / ".claude" / "hooks" / "ensure_todo" / "ensure_todo_post_tool_use.py"
        )
    ...
    def test_ensure_todo_install_removes_legacy_entries_and_scripts(self) -> None:
        ...
        legacy_post_tool_use_script = (
            self.base_path / ".claude" / "hooks" / "ensure_todo_post_tool_use.py"
        )
        ...
        self.assertEqual(
            post_tool_use_hooks[0]["hooks"][0]["command"],
            f'uv run "{post_tool_use_script.resolve()}"',
        )
    ...
    def test_legacy_only_uninstall_does_not_delete_current_ensure_todo_scripts(self) -> None:
        current_stop_script = (
            self.base_path / ".claude" / "hooks" / "ensure_todo" / "ensure_todo_stop.py"
        )
        current_post_tool_use_script = (
            self.base_path / ".claude" / "hooks" / "ensure_todo" / "ensure_todo_post_tool_use.py"
        )
        legacy_stop_script = self.base_path / ".claude" / "hooks" / "ensure_todo_stop.py"
        legacy_post_tool_use_script = (

...
class TestErrorResolutionPattern:
    ...
    def test_detects_error_followed_by_success(
        ...
        result = agent_runner.analyze_session(
            session_id="resolution-1",
            events=error_resolution_session
        )
    ...
    def test_creates_instinct_with_correct_confidence(
        ...
        assert instinct.confidence == 0.6, \
            f"Expected confidence 0.6, got {instinct.confidence}"
    ...
    def test_captures_resolution_strategy(
        ...
        assert len(instinct.action) > 10, \
            "Action description should be meaningful"
...
class TestDifferentErrorPatterns:
    ...
    def test_detects_permission_error_resolution(
        ...
        """
        permission_session = [
            {"timestamp": "2026-04-30T13:00:00Z", "event": "tool_start", "tool": "Bash", "session": "perm-1"},
            {"timestamp": "2026-04-30T13:00:05Z", "event": "tool_complete", "tool": "Bash", "session": "perm-1", "output": "error: Permission denied"},
            {"timestamp": "2026-04-30T13:00:10Z", "event": "tool_start", "tool": "Bash", "session": "perm-1", "input": {"command": "sudo chmod +x script.sh"}},
            {"timestamp": "2026-04-30T13:00:15Z", "event": "tool_complete", "tool": "Bash", "session": "perm-1", "output": "success"},
        ]

        result = agent_runner.analyze_session(
            session_id="perm-1",
            events=permission_session
        )
    ...
    def test_detects_file_not_found_resolution(
        self, agent_runner: AgentRunner
    ) -> None:
        """
        ...
        file_not_found_session = [
            {"timestamp": "2026-04-30T14:00:00Z", "event": "tool_start", "tool": "Read", "session": "fnf-1"},
            {"timestamp": "2026-04-30T14:00:05Z", "event": "tool_complete", "tool": "Read", "session": "fnf-1", "output": "error: file not found"},
            {"timestamp": "2026-04-30T14:00:10Z", "event": "tool_start", "tool": "Bash", "session": "fnf-1", "input": {"command": "find . -name file.py"}},
            {"timestamp": "2026-04-30T14:00:15Z", "event": "tool_complete", "tool": "Bash", "session": "fnf-1", "output": "./src/file.py"},
            {"timestamp": "2026-04-30T14:00:20Z", "event": "tool_start", "tool": "Read", "session": "fnf-1"},
            {"timestamp": "2026-04-30T14:00:25Z", "event": "tool_complete", "tool": "Read", "session": "fnf-1", "output": "success"},
        ]

        result = agent_runner.analyze_session(
            session_id="fnf-1",
            events=file_not_found_session
        )

        assert result is not None
        assert len(result.instincts_created) >= 1

    def test_no_false_positives_on_normal_errors(
        self, agent_runner: AgentRunner
    ) -> None:
        """
        ...
        """
        error_only_session = [
            {"timestamp": "2026-04-30T15:00:00Z", "event": "tool_start", "tool": "Bash", "session": "error-only"},
            {"timestamp": "2026-04-30T15:00:05Z", "event": "tool_complete", "tool": "Bash", "session": "error-only", "output": "error: command failed"},
            # No resolution follows
        ]

        result = agent_runner.analyze_session(
            session_id="error-only",
            events=error_only_session
        )

        # Should not create resolution instinct if there's no resolution
        if result.instincts_created:
            resolution_instincts = [
                i for i in result.instincts_created

...
name: blueprint
description: >-
  Multi-session plans from one-line objectives with per-step context briefs. Use when creating roadmaps, blueprint plans, or multi-PR architecture. Do not use for single-PR tasks.
origin: community
...
This skill ships with Everything Claude Code. No separate installation is needed when LSZ is installed.

### Full LSZ install

If you are working from the LSZ repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the LSZ diff before updating:
...
If you are vendoring only this skill outside the full LSZ install, copy the reviewed file from the LSZ repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed LSZ commit rather than running `git pull`.

...
name: toolchain-wiki
description: >-
  Native toolchain wiki router for JS/TS/Python — Oxlint/Oxfmt (oxc), Basedpyright, Worktrunk. Use when configuring linters/formatters, writing linter rules/plugins, type-checking Python, or managing parallel worktrees. TRIGGER: toolchain-wiki, oxc, oxlint, oxfmt, basedpyright, worktrunk, wt
metadata:
  manage: [oxc, typecheck, worktree]
---
...
| ----------- | ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `oxc`       | `$SKILL_DIR/subskills/oxc/SKILL.md`       | Oxlint/Oxfmt config, CLI, writing custom rules/plugins, Prettier/ESLint migration — [oxc](subskills/oxc/SKILL.md) |
...
| `worktree`  | `$SKILL_DIR/subskills/worktree/SKILL.md`  | Branch-addressed worktrees, hooks, `hash_port`, LLM commits — proxy to `worktrunk-guide`                          |

Omitted domain loads only the spine above.

## Spine

- **Runtime:** `pnpm v12` (`pnpm@12.0.0`, `.nvmrc 24`) for JS/TS, `uv` (`.python-version 3.14`) for Python, `wt` (branch-addressed) for worktrees. See `$SKILL_DIR/subskills/oxc/SKILL.md`, `programming-expert/subskills/basedpyright-expert/SKILL.md`, and `worktrunk-guide` for owners.
...
- `pnpm run lint && pnpm run format && pnpm run typecheck && pnpm test` — JS/TS gate (see `oxc`)
- `basedpyright --writebaseline && basedpyright --baselinemode=lock` — Python gate (see `typecheck`)
- `wt switch --create <branch> && wt merge` — worktree gate (see `worktree`)

## Trade-offs

- Router lean, subskills deep — on-demand `Read` keeps `TOOLCHAIN-WIKI` prompt cheap.
- One concept one location — oxc docs live in `toolchain-wiki/subskills/oxc/references/`, not duplicated in `typescript-scaffolding`.
- Native bias — `oxlint`/`oxfmt`/`basedpyright`/`wt` over JS emulation; agent latency > compatibility edge cases.

## Relation

- `scaffold` (`typescript-scaffolding`) `depends-on: [toolchain-wiki]` — TS flavor reuses `toolchain-wiki/subskills/oxc/` as its linter/formatter.
- `programming-expert` remains canonical for `basedpyright-expert`; `toolchain-wiki/subskills/typecheck/` is a proxy/index.
- `worktrunk-guide` remains canonical for `wt`; `toolchain-wiki/subskills/worktree/` is a proxy/index.

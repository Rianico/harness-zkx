...
name: docs-scraper
description: >-
  Docs pipeline: scrape LSP/PTX/CUDA/Rust/site/skill.sh to LLM markdown + layered skill generation (curated refs + raw docs). TRIGGER: scrape docs, to-skill, convert docs to skill, compose skills.
argument-hint: |-
  [scrape|to-skill] scrape: [lsp|ptx|runtime|driver|rust <target>|site|skills <url>...] [--force] [--output-dir <path>] [--base-url <url>] [--staging <path>] [--run <slug>]
...
> [!tip] Router
> This skill is a **router** with two explicit routes: `scrape` (fetch/clean/cache docs) and `to-skill` (curate layered skill). Invoke via `/skill:docs-scraper scrape ...` or `/skill:docs-scraper to-skill ...` — `argument-hint` lists both. On Pi ≥0.84.4 the skill is `disable-model-invocation:true` (true zero-load) — human or `$skill` dispatch only.
...
## Docs-to-Skill Pipeline — Layered Skill Generation

> Merged from `docs-to-skill` (retired). This is the second route of the unified `docs-scraper` skill. All implementation work is delegated to subagents; scraping is now a direct script call, not a cross-skill hop.
...
```markdown
Script (scraper is now internal — no Skill tool hop):
  uv run $SKILL_DIR/scripts/scrape.py site --base-url {URL} --output-dir {TOPIC_ROOT}/draft/skills/{SKILL_NAME}/references/{SKILL_NAME}-raw/
```
...
> [!note] Unified References
> Scrape refs: `cleanup-patterns.md`, `cli-scrape-standards.md`, `cuda-patterns.md`, `lsp-patterns.md`, `rust-patterns.md`, `section-extraction.md`, `skillsh-compose.md` · Pipeline refs (migrated from `docs-to-skill`): `module-detection.md`, `trigger-extraction.md`, `pattern-extraction.md`, `extraction-rules.md`, `skill-template.md`, `quality-metrics.md`, `compilation-contract.md` · Scripts: `scrape.py` + `compile.py`/`validate-structure.py` (pipeline) + `verify_cli_globals.py`.

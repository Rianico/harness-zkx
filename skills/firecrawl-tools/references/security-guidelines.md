...
- **File-based output isolation**: All commands use `-o` to write results to `.lsz/.firecrawl/` files rather than returning content directly into the agent's context window. This avoids overflowing the context with large web pages.
...
- **Gitignored output**: `.lsz/` is added to `.gitignore` so fetched content is never committed to version control.

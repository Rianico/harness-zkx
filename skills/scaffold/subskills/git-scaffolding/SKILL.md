...
- `.github/workflows/changelog-check.yml` — on `pull_request` to `main` (required), runs `scripts/changelog-gate.py ledger`, `fail+comment` if floor not met — SHA pins from `SHA_TABLE` (`checkout v5`, `setup-python v6`, `github-script v8`)
- `changelog-check.yml` runs `scripts/changelog-gate.py` at both boundaries: a PR run reads the `Landing: squash|merge` declaration from the PR body and checks the ledger against the landing, and the `main` run is the durable one. The same body may carry a single-line `Ledger-Waiver: <reason>` trailer, which the step forwards as `--waiver` — the recorded escape the failure comment names, and the only one: the gate waives fixable findings and never a needs-human one. The `pre-push` guard is retired — a stale `## [Unreleased]` is no longer auto-amended, because curation replaced projection.
...
- `CHANGELOG.md` `## [Unreleased]` is the ledger, gated by `scripts/changelog-gate.py` in `changelog-check.yml`; `release.yml` (`release` job) runs `scripts/changelog-unreleased.py clear` then `semantic-release` owns versioned sections (`@semantic-release/changelog` + `@semantic-release/npm` (`npmPublish: false` by default) + `@semantic-release/git`). Do not hand-edit versioned sections. Commit a sync as a hidden type (e.g. `chore: sync changelog unreleased section`) so it mints no ledger entry. Bullets are `*`; the file's MD004 pin rewrites any other style on the next markdown touch.
...
- `--only <a,b>` / `--components <a,b>` — write just those components keyed by `GIT_COMPONENTS` (`releaserc`, `release-yml`, `changelog-check`, `pre-push`, `changelog-script`, `typecheck-budget`, `commitlint`, `changelog-md`, `issue-templates`, `pr-template`, `contributing`, `agents`, `gitignore`; aliases `hooks`, `changelog`, `issues`, `pull-request`, `typecheck`). `typecheck-budget` (`scripts/typecheck-budget.py`, the basedpyright budget gate) is opt-in for git — default runs skip it unless Python is present; the python flavor always writes it.

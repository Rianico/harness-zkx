...
Yes with the [dry-run options](https://semantic-release.org/usage/configuration#dryrun) which prints to the console the next version to be published and the release notes.
...
- Use a [global](https://semantic-release.org/usage/running#using-npx-recommended) **semantic-release** installation
- Set **semantic-release** [options](https://semantic-release.org/usage/configuration#options) via [CLI arguments or Configuration file](https://semantic-release.org/usage/configuration)
- Make sure your CI job executing the `semantic-release` command has access to a version of Node that [meets our version requirement](https://semantic-release.org/support/node-version) to execute the `semantic-release` command

See the [CI configuration recipes](https://semantic-release.org/recipes/release-workflow/#ci-configurations) for more details on specific CI environments.

In addition, you will need to configure the **semantic-release** [plugins](https://semantic-release.org/foundation/plugins) to disable the [`@semantic-release/npm`](https://github.com/semantic-release/npm) plugin which is used by default and use a plugin for your project type.
...
See the [package managers and languages recipes](https://semantic-release.org/recipes/release-workflow/#package-managers-and-languages) for more details on specific project types and the [available plugins list](https://semantic-release.org/extending/plugins-list) to see if there are community-supported plugins already available for the stack you are interested in.
...
- A way to set [authentication](https://semantic-release.org/usage/ci-configuration#authentication) via environment variables
- A way to guarantee that the `semantic-release` command is [executed only after all the tests of all the jobs in the CI build pass](https://semantic-release.org/usage/ci-configuration#run-semantic-release-only-after-all-tests-succeeded)

See the [CI configuration recipes](https://semantic-release.org/recipes/release-workflow/#ci-configurations) for more details on specific CI environments.

## Can I run semantic-release on my local machine rather than on a CI server?

Yes, you can by explicitly setting the [`--no-ci` CLI option](https://semantic-release.org/usage/configuration#ci) option. You will also have to set the required [authentication](https://semantic-release.org/usage/ci-configuration#authentication) via environment variables on your local machine, for example:
...
If you need more control over the timing of releases, see [Triggering a release](https://semantic-release.org/intro#triggering-a-release) for different options.

**Note**: Only the codebase changes altering the published package will trigger a release (for example new features, bug fixes or performance improvements would trigger a release while refactoring or changing code style would not). See [How can I change the type of commits that trigger a release?](https://semantic-release.org/support/faq/#how-can-i-change-the-type-of-commits-that-trigger-a-release) for more details.
...
If your project is under heavy development, with frequent breaking changes, and is not production ready yet we recommend [publishing pre-releases](https://semantic-release.org/recipes/release-workflow/pre-releases).
...
In addition, the [verify conditions step](https://semantic-release.org/foundation/release-steps) verifies that all necessary conditions for proceeding with a release are met, and a new release will be performed [only if all your tests pass](https://semantic-release.org/usage/ci-configuration#run-semantic-release-only-after-all-tests-succeeded).

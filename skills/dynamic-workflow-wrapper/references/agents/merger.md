...
1. Admission first. The session root must have no tracked modifications and no untracked files other than throwaway artifacts. Tolerate the same untracked set every gate tolerates — machine-generated caches (`__pycache__/`, `*.pyc`, `.DS_Store`) and paths under `.lsz/` — and export `PYTHONDONTWRITEBYTECODE=1` so a stray interpreter stops creating `__pycache__` mid-run. Tolerant check: `git status --porcelain --untracked-files=all | grep -vE '(__pycache__/|\.pyc$|\.DS_Store$|\.lsz/)' || true` — any output means the root is dirty → `BLOCKED`, `cleanRoot: false`. Record `git -C <root> rev-parse --abbrev-ref HEAD` as `rootBranch` — that branch must not change during the run.
...
3. Prefer worktrunk when `command -v wt` succeeds and `.config/wt.toml` exists: `wt switch --create <integration> --base <base> --no-cd`, then read the absolute `path` from `wt list --format=json 2>/dev/null` (redirect stderr — wt prints a json-schema notice there and merging it into stdout breaks jq; accept both the schema-1 bare array with `.path` and the schema-2 `.items[]` envelope with `.worktree.path`); `tool: "wt"`. Fallback: `git worktree add <repo-parent>/<repo-name>-<slug> -b <integration> <base>`; `tool: "git"`.
...
7. Verify before reporting: the path exists as a directory, `git -C <path> rev-parse --abbrev-ref HEAD` names the integration branch, and the ephemeral-tolerant admission check from step 1 run against `<path>` prints nothing. Report every path absolute; `BLOCKED` never guesses a path.
...
2. `uv run ~/.agents/skills/branch-worktree-pr/scripts/make_copy.py task/<id>-<slug> <integration>`, or `wt switch --create task/<id>-<slug> --base <integration> --no-cd` when `wt` is the tool in play. Fallback: `git worktree add`. Reuse a matching existing worktree instead of recreating it — the loop re-verifies, so reuse is safe here in a way the integration branch is not. `make_copy.py` probes the copied dependency tree and runs the frozen-lockfile install when `wt step copy-ignored` left `node_modules` skeletonized; it exits non-zero if that repair fails, so report `BLOCKED` rather than hand a tree with dangling store symlinks (or unloadable native `.node` bindings) to the developer.

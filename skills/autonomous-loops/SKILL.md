...
**LSZ's built-in persistent loop.** A session-aware REPL that calls `claude -p` synchronously with full conversation history.
...
3. **Any loop + Verification** — Use LSZ's `/verify` command or `verification-loop` skill as a gate before commits.
...
| NanoClaw | LSZ | `/claw` command in this repo |
| Verification Loop | LSZ | `skills/verification-loop/` in this repo |

...
- **Step 6:** `eval-gate check` (Load skill: `eval-gate`) - *Run the approved eval definition against the implementation. If all required evals pass, continue to code review. If any eval fails, parse the `Issues File:` path from output and invoke `tdd-cycle --lightweight issues=[path] topic_root=[topic_root]` for test-first remediation. Retry remediation at most twice before stopping and surfacing the blocker.*
...
- **Step 2:** `eval-gate define` (Load skill: `eval-gate`) - *Pass the approved `design.md` and `topic_root=[topic_root]` override. No ADR or plan required. The eval definition is derived from the design only, typically capability and contract checks for the single function/module. Stop for explicit user review and approval before implementation begins.*
...
- **Step 1:** `tdd-cycle` (Load skill: `tdd-cycle`) - *Create `[topic_root]` once for the topic, then pass `topic_root=[topic_root]` to write a failing test for the bug and fix it.*
...
- **Step 3:** `eval-gate check` (Load skill: `eval-gate`) - *Execute if an eval already exists for the affected behavior or if the bugfix changes an externally visible contract. If any eval fails, parse the `Issues File:` path from output and invoke `tdd-cycle --lightweight issues=[path] topic_root=[topic_root]` for test-first remediation. Do not create a new ADR for ordinary bugfixes.*

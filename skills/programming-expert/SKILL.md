...
- **Design deepening vs. scope creep** — distinguish internal structural refactoring from unmandated scope expansion. Scope creep adds unrequested public API methods, mutates payload schemas, adds user-visible parameters, or modifies repo tooling/configs without mandate. Design deepening decomposes monolithic modules to cure code smells (Divergent Change, Shotgun Surgery, God modules), extracts cohesive internal helpers, and adds architectural/contract regression tests (`test/arch/`). Design deepening cures smells and strengthens invariants without widening public surfaces; never treat modular refactorings as scope creep.

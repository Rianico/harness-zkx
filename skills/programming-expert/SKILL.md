...
- **Resilience — retry only transient** — retry `ConnectionError/TimeoutError` + `429/502/503/504`, never `ValueError/TypeError`/auth 4xx; `wait_exponential_jitter` with cap on attempts *and* wall time; jitter to avoid thundering herd.

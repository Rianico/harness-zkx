...
def run_cmd(cmd: str) -> str:
    """Run allowlisted diagnostic command without shell."""
    allowed = ("npx tsc", "node -v", "grep -r", "grep -rn")
    if not cmd.startswith(allowed):
        return f"Blocked unallowlisted command: {cmd[:80]}"
    clean = cmd.replace(" 2>/dev/null", "").replace(" 2>&1", "")
    if " | " in clean:
        ...
        try:
            ...
            for pipe in parts[1:]:
                if pipe.startswith("head -"):
                    try:
                        n = int(pipe.split("-", 1)[1].strip().split()[0])
                    except ValueError:
                        n = 20
                    ...
                    if "'" in pipe:
                        ...
                        pat = pipe.split()[-1]
                    lines = output.splitlines()
                    output = "\n".join(line for line in lines if pat not in line)
                elif pipe.startswith("grep -E"):
                    if "'" in pipe:
                        pat = pipe.split("'")[1]
                    elif '"' in pipe:
                        pat = pipe.split('"')[1]

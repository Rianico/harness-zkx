...
All scripts source the skill's shared modules: `lib/log.sh` for `phase`/`ok`/`warn`/`fail`/`step` helpers with ANSI (respects `NO_COLOR`), and `lib/repo.sh` for push-remote repo identity (`repo_slug`, `default_branch`). `release-watch.sh` sources only `lib/repo.sh` — it needs the fact, not the presentation. No duplicate dry-run; no `npm` prefix noise; no `--verbose` unless failed. Set `GH_RELEASE_QUIET=1` to suppress informational lines (`info`/`dim`/`step` + banner bar) — only `ok`/`warn`/`fail`/`phase` markers print, cutting each script to ~5 lines for context-limited harness runs.

...
- **Step 7:** `code-review` (Load skill: `code-review`) - *Pass `topic_root=[topic_root]`, approved upstream artifact pointers, and eval pass log. This is the repository-level review gate for security, maintainability, broader correctness gaps, and overall readiness after implementation and eval verification.*
...
- **Step 5:** `code-review` (Load skill: `code-review`) - *Pass `topic_root=[topic_root]` plus any test, build, eval, or ADR pointers created during the fix.*
...
- **Step 2:** `code-review` (Load skill: `code-review`) - *Pass `topic_root=[topic_root]`. Document-only work normally does not require ADR or eval unless it changes a normative workflow contract.*

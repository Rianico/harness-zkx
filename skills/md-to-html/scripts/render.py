...
class BodyRenderer:
    ...
    def __init__(self, mappings: dict[str, str] | None = None) -> None:
        ...
        """
        self.mappings: dict[str, str] = mappings if mappings is not None else dict(self._DEFAULT_MAPPINGS)
            ...
            if current_section:
                ...
                if el.name == 'blockquote' and '[!' in text:
                    current_card = self._process_callouts(
                        new_soup, el, current_section, current_card,
                        strength_map, category_map, glossary,
                    )
                    continue
            ...
            if kind == 'badge':
                ...
                for t in tags_inner:
                    ...
                    item = new_soup.new_tag(
                        'span', attrs={'class': f'md-legend-key-item legend-tag-{t_key}'},
                    )
    ...
    def _color_enums_and_link_tables(self, new_soup: BeautifulSoup, strength_map: dict, category_map: dict) -> None:
        ...
        for table in new_soup.find_all('table'):
            ...
            for tr in table.find_all('tr')[1:]:
                ...
                if candidate_idx is not None and candidate_idx < len(tds):
                    ...
                    if name:
                        td.clear()
                        a = new_soup.new_tag('a', attrs={'href': f'#{slugify(name)}', 'class': 'md-overview-link'})
                        a.string = name
                        td.append(a)
...
class AssetBundler:
    ...
    def bundle(self, soup: BeautifulSoup, doc: ParsedDocument, output_path: str | None) -> str:
        ...
        head.append(result.new_tag('meta', attrs={'name': 'viewport', 'content': 'width=device-width, initial-scale=1'}))
        ...
        header = self._build_header(
            result, frontmatter, strength_map, category_map, legend_data, glossary,
        )
    ...
    def _build_header(self, doc, frontmatter: dict, strength_map: dict, category_map: dict, legend_data: dict, glossary: dict):

...
- [`backend`](backend.md) — This module provides the backend implementations for different terminal libraries.
- [`buffer`](buffer.md) — A module for the [`Buffer`] and [`Cell`] types.
- [`layout`](layout.md) — Layout and positioning in terminal user interfaces.
- [`style`](style.md) — `style` contains the primitives used to control how your user interface will look.
- [`symbols`](symbols.md) — Symbols and markers for drawing various widgets.
- [`terminal`](terminal.md) — Provides the [`Terminal`], [`Frame`], [`CompletedFrame`], and [`Viewport`] types.
- [`text`](text.md) — Primitives for styled text.
- [`widgets`](widgets.md) — The `widgets` module contains the `Widget` and `StatefulWidget` traits, which are used to

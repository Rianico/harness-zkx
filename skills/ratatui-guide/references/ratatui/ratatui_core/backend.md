...
Note: most applications should use the [`Terminal`](./terminal.md) struct instead of directly calling methods
on the backend.
...
[`Terminal`](./terminal.md) struct provides a higher level interface for interacting with the terminal.

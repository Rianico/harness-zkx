*[ratatui_widgets](./index.md) / [gauge](#)*
...
- `fn block(self, block: Block<'a>) -> Self` — [`Block`](./block.md#block)
  ...
  Surrounds the `Gauge` with a [`Block`](./block.md).
...
- `fn style<S: Into<Style>>(self, style: S) -> Self`
  ...
  except the bar itself). [`Block`](./block.md) style set with `Gauge::block` takes precedence.
...
- `fn block(self, block: Block<'a>) -> Self` — [`Block`](./block.md#block)
  ...
  Surrounds the `LineGauge` with a [`Block`](./block.md).
...
- `const fn line_set(self, set: symbols::line::Set<'a>) -> Self`
  ...
  See [`symbols::line::Set`](../ratatui_core/symbols/block.md) for more information. Predefined sets are also available, see

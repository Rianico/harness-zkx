...
- Use [`run()`](init/index.md) for normal applications. It initializes the terminal, runs your app, and
  restores the terminal on exit.
- Use [`init()`](init/index.md) / [`restore()`](init/index.md) (or [`try_init()`](init/index.md) / [`try_restore()`](init/index.md)) when you want manual
  control over terminal lifetime and the event loop structure.
- Use [`init_with_options()`](init/index.md) / [`try_init_with_options()`](init/index.md) when you need a custom [`Viewport`](#viewport),
...
that for each frame, your app must render all [`widgets`](widgets/index.md) that are supposed to be part of the
...
The simplest way to initialize and run a terminal application is to use the [`run()`](init/index.md) function,
...
For more control over initialization and restoration, you can use [`init()`](init/index.md) and [`restore()`](init/index.md):
...
Use [`run()`](init/index.md) as the default. Reach for [`init()`](init/index.md) / [`restore()`](init/index.md) when setup and teardown
...
Before the convenience functions were introduced in version 0.28.1 ([`init()`](init/index.md)/[`restore()`](init/index.md))
and 0.30.0 ([`run()`](init/index.md)), applications constructed [`Terminal`](prelude/index.md) and [`Backend`](../ratatui_core/backend/index.md) instances
...
convenience helpers. See the [`Terminal`](prelude/index.md) and [`backend`](backend/index.md) module documentation for details.
...
method takes a closure that is called with a [`Frame`](prelude/index.md) instance. The [`Frame`](prelude/index.md) provides the size
...
- **If you move the cursor directly:**<br> Cursor changes made through backend-specific APIs or
  [`Terminal`](prelude/index.md) cursor methods can be overwritten by the next render pass if that pass sets
  cursor state through [`Frame`](prelude/index.md). Prefer choosing one path and using it consistently.
...
important one is [`Style`](../ratatui_core/style/index.md) which represents the foreground and background colors and the text
...
- [`backend`](backend/index.md) — Re-exports for the backend implementations.
- [`prelude`](prelude/index.md) — A prelude for conveniently writing applications using this library.
- [`widgets`](widgets/index.md) — Widgets are the building blocks of user interfaces in Ratatui.
- [`init`](init/index.md) — Terminal initialization and restoration functions.
...
A trait for converting a value to a [`Span`](prelude/index.md).
...
Get the size of the terminal screen in columns/rows as a [`Size`](../ratatui_widgets/chart/index.md).

The returned [`Size`](../ratatui_widgets/chart/index.md) contains the width and height of the terminal screen.
...
This will create a new [`DefaultTerminal`](init/index.md) and initialize it with the following defaults:
...
Generally, use this function instead of [`try_init`](init/index.md) to ensure that the terminal is restored
correctly if any of the initialization steps fail. If you need to handle the error yourself, use
[`try_init`](init/index.md) instead.
...
will create a new [`DefaultTerminal`](init/index.md) and initialize it with the given options and the following
defaults:

- Raw mode is enabled
- A panic hook is installed that restores the terminal before panicking.

Unlike [`init`](init/index.md), this function does not enter the alternate screen buffer as this may not be
...
Generally, use this function instead of [`try_init_with_options`](init/index.md) to ensure that the terminal is
restored correctly if any of the initialization steps fail. If you need to handle the error
yourself, use [`try_init_with_options`](init/index.md) instead.
...
Use this function over [`try_restore`](init/index.md) when you don't need to handle the error yourself, as
ignoring the error is generally the correct behavior when cleaning up before exiting. If you
need to handle the error yourself, use [`try_restore`](init/index.md) instead.
...
This function creates a new [`DefaultTerminal`](init/index.md) with [`init`](init/index.md) and then runs the given closure
with a mutable reference to the terminal. After the closure completes, the terminal is restored
to its original state with [`restore`](init/index.md).

This function is a convenience wrapper around [`init`](init/index.md) and [`restore`](init/index.md), and is useful for simple
...
This function will attempt to create a [`DefaultTerminal`](init/index.md) and initialize it with the following
defaults:
...
Generally, you should use [`init`](init/index.md) instead of this function, as the panic hook installed by this
...
This function allows the caller to specify a custom [`Viewport`](../ratatui_core/index.md) via the [`TerminalOptions`](#terminaloptions). It
will attempt to create a [`DefaultTerminal`](init/index.md) and initialize it with the given options and the
following defaults:

- Raw mode is enabled
- A panic hook is installed that restores the terminal before panicking.

Unlike [`try_init`](init/index.md), this function does not enter the alternate screen buffer as this may not be
desired in all cases. If you need the alternate screen buffer, you should enable it manually
after calling this function.
...
Generally, you should use [`init_with_options`](init/index.md) instead of this function, as the panic hook
...
Use [`restore`](init/index.md) instead of this function when you don't need to handle the error yourself, as
ignoring the error is generally the correct behavior when cleaning up before exiting. If you
...
Returns a reference to the [`Cell`](../ratatui_widgets/borders/index.md) at the given position.

This method accepts any value that can be converted to [`Position`](../ratatui_core/backend/index.md) (e.g. `(x, y)` or
`Position::new(x, y)`).

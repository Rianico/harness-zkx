...
| 2     | `uv run scripts/make_copy.py <child-branch> <base-branch>` | `wt switch --create <child> --base <base> --no-cd --yes`, check `wt list` exact branch match and `git -C <path> branch --show-current`, probe the copied dependency tree and run the frozen-lockfile install when `wt step copy-ignored` left it skeletonized (`--skip-deps` / `--force-deps`), print absolute path. Stays on parent folder. |
...
- `wt_list() -> list[Worktree]` — `wt list --format=json` (parsed, validated `Worktree(branch, path, is_current)`) fallback to `git worktree list --porcelain` (correct `worktree <path>` / `branch refs/heads/<name>` parsing). Accepts BOTH JSON schemas — the schema-1 bare array (row-level `path` / `is_current`) and the schema-2 `{ items: [...] }` envelope (`worktree.path` / `worktree.current`) — because `[list] json-schema` is a user-config key wt ignores in project config. stderr is captured separately, so the schema deprecation notice never reaches the parser. Rows without a path are skipped.
- `git_status_clean(allow=[.lsz/tmp, tmp/pi-open-tui]) -> tuple[bool, list[str]]` — also tolerates machine-generated caches (`__pycache__/`, `*.pyc`, `.DS_Store`) via `is_ephemeral_path()`; a stray interpreter without `PYTHONDONTWRITEBYTECODE=1` must not fail an admission gate.
- `detect_install_command(cwd?) -> list[str] | None` — first lockfile found wins: `pnpm-lock.yaml` → `pnpm install --frozen-lockfile`, `package-lock.json` → `npm ci`, `yarn.lock` → `yarn install --frozen-lockfile`, `bun.lock(b)` → `bun install --frozen-lockfile`, `uv.lock` → `uv sync --frozen`, `Cargo.lock` → `cargo fetch --locked`; else `None`.
- `dependency_tree_healthy(cwd?) -> tuple[bool, str]` — resolves top-level `node_modules` symlinks, so a `wt step copy-ignored` skeleton with dangling store links (and unloadable native `.node` bindings) is detected. `ensure_dependencies(cwd?, force=False) -> tuple[bool, str]` runs the frozen-lockfile install when the probe fails; a missing package manager is a no-op, a failed install is a hard failure.
- `run(cmd: list[str], cwd?)`, `ensure_clean_worktree()`, `current_branch()`, `run_gate(gate, cwd?)` (trusted `shell=True` only for `wt.toml` gate).

...
| 3     | `uv run scripts/merge_copy.py <copy-path> <target-branch>` | Thin router: `wt merge -C <absolute-copy-path> --stage tracked <target> --yes`; detect `rebase incomplete` / `Unmerged paths` → conflict exit 2 (needs fixer `Resolve merge conflicts in copy at <path>`), else gate fail exit 1. No hunk fixing or `rebase --continue` here — that belongs to fixer via `resolving-merge-conflicts`. |

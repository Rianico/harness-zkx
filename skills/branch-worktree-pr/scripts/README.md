...
| 3 | `uv run scripts/merge_copy.py <copy-path> <target-branch>` | `wt merge <target> --yes` inside copy. If conflict, fix **inside that copy only**: `git status` for `DU` → `git rm` keep deletion, `UU`/`AA` → check no `<<<<<<<` markers then `git add`, run gate via `read_gate()` (trusted) or `wt step pre-merge`, then `GIT_EDITOR=true git rebase --continue` or `git commit --no-edit` as needed and retry merge. Main folder never edited. |
...
| — | `uv run scripts/worktree.py <subcommand>` | Dispatcher forwarding to same impl modules: `claim`, `create-target`, `make-copy`, `merge-copy`, `verify`, `check-history`, `open-pr`. Also available at `uv run scripts/worktree.py` (root). |
...
- Rebase always with `GIT_EDITOR=true` to avoid editor.
...
- Exit code 0 = ok, non-zero = blocked/failed — treat as blocker.

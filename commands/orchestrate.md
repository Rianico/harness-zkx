...
Whenever a Command's execution instructions require you to launch an agent or pass state, you MUST pass the file path pointer instead of reading the artifact directly into the context window. Prepend the `[DOMAIN CONTEXT]` to the prompt so the sub-agent inherits your discovery, ALONG WITH the absolute file path (pointer) output from the previous pipeline step:
- `prompt`: "**[DOMAIN CONTEXT]**\nLanguage/Domain: [e.g., Rust]\nRoot File: [e.g., Cargo.toml]\n\n**[PREVIOUS STATE POINTER]**\n[Include the absolute file path of the approved artifact from the previous step]\n\n**[TASK]**\n[task summarization and user requirements]"

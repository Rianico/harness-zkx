...
function stateFilePath() {
  if (process.env.LSZ_MCP_HEALTH_STATE_PATH) {
    return path.resolve(process.env.LSZ_MCP_HEALTH_STATE_PATH);
  }
  return path.join(os.homedir(), '.claude', 'mcp-health-cache.json');
}

function configPaths() {
  if (process.env.LSZ_MCP_CONFIG_PATH) {
    return process.env.LSZ_MCP_CONFIG_PATH
...
function readRawStdin() {
  return new Promise(resolve => {
    let raw = '';
    let truncated = /^(1|true|yes)$/i.test(String(process.env.LSZ_HOOK_INPUT_TRUNCATED || ''));
...
function markHealthy(state, serverName, now, details = {}) {
  state.servers[serverName] = {
    status: 'healthy',
    checkedAt: now,
    expiresAt: now + envNumber('LSZ_MCP_HEALTH_TTL_MS', DEFAULT_TTL_MS),
    failureCount: 0,
    lastError: null,
...
function markUnhealthy(state, serverName, now, failureCode, errorMessage) {
  ...
  const backoffBase = envNumber('LSZ_MCP_HEALTH_BACKOFF_MS', DEFAULT_BACKOFF_MS);
...
function probeCommandServer(serverName, config) {
  return new Promise(resolve => {
    ...
    const timeoutMs = envNumber('LSZ_MCP_HEALTH_TIMEOUT_MS', DEFAULT_TIMEOUT_MS);
    const mergedEnv = {
      ...process.env,
...
async function probeServer(serverName, resolvedConfig) {
  ...
  if (config.type === 'http' || config.url) {
    const result = await requestHttp(config.url, config.headers || {}, envNumber('LSZ_MCP_HEALTH_TIMEOUT_MS', DEFAULT_TIMEOUT_MS));
...
function reconnectCommand(serverName) {
  const key = `LSZ_MCP_RECONNECT_${String(serverName).toUpperCase().replace(/[^A-Z0-9]/g, '_')}`;
  const command = process.env[key] || process.env.LSZ_MCP_RECONNECT_COMMAND || '';
  if (!command.trim()) {
    return null;
  }
...
function attemptReconnect(serverName) {
  ...
  const result = spawnSync(command, {
    ...
    cwd: process.cwd(),
    encoding: 'utf8',
    timeout: envNumber('LSZ_MCP_RECONNECT_TIMEOUT_MS', DEFAULT_TIMEOUT_MS)
  });
...
function shouldFailOpen() {
  return /^(1|true|yes)$/i.test(String(process.env.LSZ_MCP_HEALTH_FAIL_OPEN || ''));
}
...
async function main() {
  ...
  if (truncated) {
    const limit = Number(process.env.LSZ_HOOK_INPUT_MAX_BYTES) || MAX_STDIN;
    const logs = [
      shouldFailOpen()

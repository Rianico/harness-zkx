 ...
 *
 * Enable: Set LSZ_GOVERNANCE_CAPTURE=1
 * Configure session: Set LSZ_SESSION_ID for session correlation
 */
...
function run(rawInput, options = {}) {
  ...
  if (String(process.env.LSZ_GOVERNANCE_CAPTURE || '').toLowerCase() !== '1') {
    return rawInput;
  }

  const sessionId = process.env.LSZ_SESSION_ID || null;
...
if (require.main === module) {
  ...
  process.stdin.on('end', () => {
    const result = run(raw, {
      truncated,
      maxStdin: Number(process.env.LSZ_HOOK_INPUT_MAX_BYTES) || MAX_STDIN,
    });
